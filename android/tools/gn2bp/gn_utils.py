...
def _get_jni_registration_deps(gn_target_name, gn_desc):
  # See https://source.chromium.org/chromium/chromium/src/+/main:third_party/jni_zero/jni_zero.gni;l=117;bpv=1
...
class GnParser(object):
  ...
  def parse_gn_desc(self, gn_desc, gn_target_name, java_group_name=None, is_test_target=False):
    ...
    if desc.get("script", "") == "//tools/protoc_wrapper/protoc_wrapper.py":
      ...
      # the target is of type `generate_jni_registration` otherwise it will return
      # an empty set.
      # generate_jni_registration target has no deps as seen
      # https://source.chromium.org/chromium/chromium/src/+/main:third_party/jni_zero/jni_zero.gni;l=126;bpv=1
      # the dependencies are stored within another target with the same name
      # and a __java_sources suffix.
      ...
      # See https://source.chromium.org/chromium/chromium/src/+/main:third_party/jni_zero/jni_zero.gni;l=421?q=jni_source_files%20-path:out&ss=chromium%2Fchromium%2Fsrc
      # for more details
