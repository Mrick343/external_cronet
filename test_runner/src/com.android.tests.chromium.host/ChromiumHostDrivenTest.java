...
public class ChromiumHostDrivenTest implements IRemoteTest, IDeviceTest, ITestCollector,
     ...
     * location
     *
     * It is necessary to create a temp file for output for each instrumentation run and not module
     * invocation.
     * This is preferred over using {@link com.android.tradefed.targetprep.RunCommandTargetPreparer}
     * because
     * RunCommandTargetPreparer is only run once before the test invocation which leads to incorrect
     * parsing
     * as the retries will all use the same file for test result outputs.
     */
    private String createTempResultFileOnDevice() throws IOException, DeviceNotAvailableException {
        File resultFile = FileUtil.createTempFile("gtest_results", ".txt");
        String deviceFileDestination = String.format("/data/local/tmp/%s", resultFile.getName());
    ...
    private String getGTestFilters() throws DeviceNotAvailableException {
        ...
        if (!includeFilters.isEmpty() || !excludeFilters.isEmpty()) {
            ...
            if (!includeFilters.isEmpty()) {
                filter.append(ArrayUtil.join(":", includeFilters));
            }
            if (!excludeFilters.isEmpty()) {
                filter.append("-");
                filter.append(ArrayUtil.join(":", excludeFilters));
            }
        }
     ...
     */
    private String createFlagFileOnDevice(String filter) throws DeviceNotAvailableException {
        ...
        String devicePath = String.format("/data/local/tmp/%s", tmpFlagFile.getName());
        try {
            if (!getDevice().pushFile(tmpFlagFile, devicePath)) {
     ...
     * started by the instrumentation APK. This activity is responsible for executing gtests.</li>
     * <li> RUN_IN_SUBTHREAD_KEY: Whether to run the tests in the main-thread or a sub-thread.</li>
     * <li> EXTRA_SHARD_NANO_TIMEOUT_KEY: Shard timeout (Equal to the test timeout and not
     * important as we only use a single shard).</li>
     * <li> LIBRARY_TO_LOAD_ACTIVITY_KEY: Name of the native library which has the code under
     * test. System.LoadLibrary will be invoked on the value of this flag</li>
     ...
     * mostly used for gtest flags</li>
     ...
     */
    private String createRunAllTestsCommand(String resultFilePath)
    ...
    public void run(TestInformation testInfo, ITestInvocationListener listener)
        ...
        try {
            resultFilePath = createTempResultFileOnDevice();
        } catch (IOException e) {
            throw new FailedChromiumGtestException(
                    "Failed to create temp file for result on the device.", e.getCause());
        }
        ...
        try {
            ...
            throw new FailedChromiumGtestException("Failed to parse and report test results.",
                    e.getCause());
        }
    }

    private void parseAndReport(String resultFilePath, ITestInvocationListener listener)
            throws IOException, DeviceNotAvailableException {
        File resultFile = device.pullFile(resultFilePath);
        if (resultFile == null) {
            throw new IOException(
                    String.format("Failed to retrieve %s from device", resultFilePath));
        }
        // Loading all the lines is fine since this is done on the host-machine.
        String[] lines = Files.readAllLines(resultFile.toPath()).toArray(String[]::new);
