    ...
    public void run(TestInformation testInfo, ITestInvocationListener listener)
        ...
        try {
            ...
            getDevice().executeShellCommand(cmd, new CollectingOutputReceiver(),
                    testsTimeout.toMinutes(), TimeUnit.MINUTES, 1);
            parseAndReport(getDevice().pullFile(GTEST_OUTPUT_PATH), listener);
        } catch (Exception e) {
            // Throw a runtime otherwise the infra will report no tests run.
            throw new RuntimeException(e);
        }
    }

    private void parseAndReport(File file, ITestInvocationListener listener) throws IOException {
        if (file == null) {
            return;
        }
        ...
        if (isCollectTestsOnly()) {
            GTestListTestParser parser = new GTestListTestParser(libraryToLoad, listener);
            parser.processNewLines(lines);
            parser.done();
        } else {
            GTestResultParser parser = new GTestResultParser(libraryToLoad, listener);
            parser.processNewLines(lines);
            parser.done();
        }
    }
